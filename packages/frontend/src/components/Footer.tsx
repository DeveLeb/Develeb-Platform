...
const social_media = [
  ...
  {
    element: (
      <Image src="/icons/twitter-icon.png" width={18} height={18} alt="twt" />
    ),
  },
];
function Footer() {
  return (
    <footer className="flex flex-wrap flex-col bg-primary text-white text-sm p-10 ml:pb-20 ml:flex-row xl:px-20">
      <div className="links-container flex flex-col justify-center gap-6 order-2 pb-8  ml:flex-row ml:w-[65%] ml:gap-0 ml:border-b ml:border-gray-500 ml:mb-[30px] ml:pb-0 ml:order-none">
        {links_sections.map(
          (section: { title: string; links: string[] }, key) => {
      ...
      <div className="mb-4 border border-border px-2 py-1 text-center order-1 ml:mb-0 ml:order-none ">
        logo
      </div>
